...
## data extracted from 
# https://plot.ly/~MattSundquist/2404/exports-and-imports-to-and-
# from-denmark-norway-from-1700-to-1780/#plot
...
# ggplot2 -----------------------------------------------------------------
ggplot(molten_data, aes(x = year, 
                        y = value)) +
  geom_line(aes(col = variable), size = 1) +
  geom_ribbon(aes(ymin = min, 
                  ymax = value, 
                  fill = variable), alpha = 0.3) +
  scale_color_manual(values = c("darkred", "gold3"), 
                     guide = F) +
  scale_fill_manual(values = c("#90752d", "#BB5766"), 
                    guide = F) +
  theme_bw() +
  annotate("text", 
           x = year[5], 
  ...
  scale_x_date(breaks = seq(year[1], 
                            year[18], 
                            by = "10 years"),
               labels = format(seq(year[1], 
                                   year[18], 
                                   by = "10 years"), "%Y")) +
